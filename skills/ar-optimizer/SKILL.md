...
**Carve-out, do NOT flag mid-flow / conversational action skills.** A skill that the orchestrator (or the model mid-task) is expected to invoke programmatically must set `disable-model-invocation: false` on purpose, so model-invocability is correct, not a defect. Treat `false` as intentional for: skills the orchestrator hands off to during a run (`ar-record-improvement`, `ar-taskflow-review`, `ar-taskflow-fix-comments`) and conversational action skills the model should fire when the user asks (`a_sk_commit`, `a_sk_pr`). Only flag a `true`/missing flag on a skill that is genuinely user-only (setup/destructive/standalone, e.g. `ar-init-*`, `ar-install`, `ar-upgrade`).
